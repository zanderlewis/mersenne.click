<div>
    <header
        class="py-12 bg-black bg-opacity-30 backdrop-filter backdrop-blur-lg border-b border-purple-300/40 rounded-b-2xl">
        <div class="container mx-auto px-6 pb-6">
            <h1
                class="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600">
                mersenne.click
            </h1>
            <p class="mt-4 text-lg text-purple-200 font-light">
                Centralized information on Mersenne Primes
            </p>
        </div>
        <x-nav />
        <a href="https://github.com/zanderlewis/mersenne.click" target="_blank" class="absolute top-4 right-4 h-8 w-8">
            <x-github />
        </a>
    </header>
    <x-banner />
</div>
